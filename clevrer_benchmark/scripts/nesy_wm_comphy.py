"""NeSy-WM full stack on ComPhy -- the benchmark that originally showed ZERO transfer.

Module 1: IPEv2 (relative-invariant + force-prior), meta-trained on CLEVRER+ComPhy,
          which generalizes to ComPhy (neutral +55%).
Module 2: its rollout -> discrete collision fact per pair -> text.
Module 3: the LM (LoRA) reads question + fact and answers.

We then ablate the readout (full / no-readout / scrambled) to test whether the
LM's ComPhy dynamical answer is causally bound to the (now generalizing) simulator.
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

import h5py
import numpy as np
import torch
import torch.nn.functional as F

sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))
from clevrer_benchmark.run_adapter_evaluation import load_adapter_model  # noqa: E402
from clevrer_benchmark.scripts.ipe_v2_transfer import IPEv2  # noqa: E402
from clevrer_benchmark.scripts.velocity_finetune import score_choices  # noqa: E402
from physics_llm_adapter.intuitive_physics_engine import rollout_loss  # noqa: E402
from clevrer_benchmark.scripts.nesy_wm import _desc, prompt  # noqa: E402

# --- reproducibility: resolve data/checkpoint paths from env vars (see REPRODUCTION.md) ---
import os as _os_repro
from pathlib import Path as _Path_repro
_REPRO_ROOT = _Path_repro(__file__).resolve().parents[2]

CLEVRER_H5 = _os_repro.environ.get("CLEVRER_H5", str(_REPRO_ROOT / "data" / "clevrer_training_expanded.h5"))
DEFAULT_CKPT = _os_repro.environ.get("ADAPTER_CKPT", str(_REPRO_ROOT / "checkpoints" / "adapter_phase3.pt"))
COMPHY_CACHE = "comphy_benchmark/results/ipe_comphy_cache.npz"
T0P, HOR, THRESH = 45, 8, 0.9


def collide(pos_seq, i, j, d0):
    if d0 < THRESH:
        return None
    return bool(np.linalg.norm(pos_seq[:, i] - pos_seq[:, j], axis=-1).min() < THRESH)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--ipe_steps", type=int, default=1800)
    ap.add_argument("--lm_steps", type=int, default=1000)
    args = ap.parse_args()
    dev = args.device if torch.cuda.is_available() else "cpu"

    # ---- Module 1: meta-train IPEv2 on CLEVRER + ComPhy ----
    with h5py.File(CLEVRER_H5, "r") as f:
        Sc = np.asarray(f["states"][:14000]); Mc = np.asarray(f["masks"][:14000, 0, :])
    t0c = Sc.shape[1] // 2
    Sc_t, Mc_t = torch.from_numpy(Sc).float(), torch.from_numpy(Mc).float()
    d = np.load(COMPHY_CACHE)
    S, M, CH = d["states"], d["masks"], d["charged"]
    perm = np.random.default_rng(0).permutation(len(S)); S, M, CH = S[perm], M[perm], CH[perm]
    nqa = int(0.8 * len(S))                       # QA train / eval split on ComPhy
    Sp_t, Mp_t = torch.from_numpy(S).float(), torch.from_numpy(M[:, T0P, :]).float()

    ipe = IPEv2().to(dev); opt = torch.optim.Adam(ipe.parameters(), lr=3e-4)
    g = torch.Generator().manual_seed(0)
    print("Module 1: meta-training IPEv2 (CLEVRER+ComPhy)...")
    ipe.train()
    for step in range(1, args.ipe_steps + 1):
        if step % 2 == 0:
            bi = torch.randint(0, Sc.shape[0], (96,), generator=g)
            s, m, t0 = Sc_t[bi].to(dev), Mc_t[bi].to(dev), t0c
        else:
            bi = torch.randint(0, nqa, (96,), generator=g)
            s, m, t0 = Sp_t[bi].to(dev), Mp_t[bi].to(dev), T0P
        pred, _ = ipe(s, m, t0, 8, 8); loss = rollout_loss(pred, s, m, t0, 8)
        opt.zero_grad(); loss.backward()
        torch.nn.utils.clip_grad_norm_(ipe.parameters(), 1.0); opt.step()
    ipe.eval()

    # ---- precompute per-scene (pair, gold, IPEv2 readout) on ComPhy ----
    print("Module 2: precomputing IPEv2 collision readouts on ComPhy...")
    recs = []  # (scene_idx, di, dj, gold, ipe_pred)
    with torch.no_grad():
        for s0 in range(0, len(S), 256):
            sl = slice(s0, min(s0 + 256, len(S)))
            ss = torch.from_numpy(S[sl]).float().to(dev)
            mm = torch.from_numpy(M[sl, T0P, :]).float().to(dev)
            pred, _ = ipe(ss, mm, T0P, 8, HOR)
            ppos = pred[:, :, :, 0:3].cpu().numpy()           # [B,HOR,N,3]
            for b in range(ppos.shape[0]):
                gi = s0 + b; frame = S[gi, T0P]; mk = M[gi, T0P, :]
                idx = np.where(mk > 0)[0]
                pair = None
                for a in range(len(idx)):
                    for c in range(a + 1, len(idx)):
                        i, j = idx[a], idx[c]
                        di, dj = _desc(frame[i]), _desc(frame[j])
                        if di == dj:
                            continue
                        d0 = float(np.linalg.norm(frame[i][0:3] - frame[j][0:3]))
                        gold = collide(S[gi, T0P + 1:T0P + 1 + HOR, :, 0:3], i, j, d0)
                        if gold is None:
                            continue
                        ipe_c = collide(ppos[b], i, j, d0)
                        pair = (gi, di, dj, int(gold), int(bool(ipe_c))); break
                    if pair:
                        break
                if pair:
                    recs.append(pair)
    tr = [r for r in recs if r[0] < nqa]; ev = [r for r in recs if r[0] >= nqa]
    print(f"  usable QA scenes: train={len(tr)} eval={len(ev)}")

    # ---- Module 3: LM that queries the readout ----
    adapter = load_adapter_model(DEFAULT_CKPT, "", device=dev)
    for n, p in adapter.named_parameters():
        p.requires_grad = "lora" in n.lower()
    trainable = [p for p in adapter.parameters() if p.requires_grad]

    def st(gi):
        s = torch.from_numpy(S[gi, T0P:T0P + 1].copy()).float().unsqueeze(0).to(dev)
        m = torch.from_numpy(M[gi, T0P, :]).float().unsqueeze(0).to(dev)
        return s, m

    def evaluate(tag):
        cond = {"full": 0, "no-readout": 0, "scrambled": 0}; tot = 0
        for gi, di, dj, gold, ipe_c in ev:
            s, m = st(gi)
            for name, fact in [("full", "will" if ipe_c else "will not"),
                               ("no-readout", None),
                               ("scrambled", "will not" if ipe_c else "will")]:
                q = prompt(di, dj, fact)
                pr = int(score_choices(adapter, s, m, q, ["yes", "no"], dev).argmax())
                cond[name] += int((pr == 0) == bool(gold))
            tot += 1
        oc = sum(int(r[3] == r[4]) for r in ev)
        print(f"[{tag}] n={tot}  IPEv2-vs-actual={100*oc/max(tot,1):.1f}%  " +
              "  ".join(f"{k}={100*v/max(tot,1):.1f}%" for k, v in cond.items()))

    evaluate("BEFORE")
    opt2 = torch.optim.AdamW(trainable, lr=3e-4); rng = np.random.default_rng(0)
    step = 0; opt2.zero_grad(); losses = []
    while step < args.lm_steps and tr:
        gi, di, dj, gold, ipe_c = tr[int(rng.integers(len(tr)))]
        s, m = st(gi)
        q = prompt(di, dj, "will" if ipe_c else "will not")
        sc = score_choices(adapter, s, m, q, ["yes", "no"], dev, grad=True)
        loss = F.cross_entropy(sc.unsqueeze(0), torch.tensor([0 if gold else 1], device=dev))
        (loss / 4).backward(); losses.append(loss.item()); step += 1
        if step % 4 == 0:
            torch.nn.utils.clip_grad_norm_(trainable, 1.0); opt2.step(); opt2.zero_grad()
        if step % 400 == 0:
            print(f"  LM step {step}  CE {np.mean(losses[-400:]):.3f}")
    evaluate("AFTER")


if __name__ == "__main__":
    main()
