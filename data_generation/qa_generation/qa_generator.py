...
class PhysicsQAGenerator:
    ...
    # Default (legacy) state-tensor layout used by Phase 1-2 synthetic
    # physics datasets and the Phase A-D Isaac/PyBullet rollouts. The
    # CLEVRER scene-converter produces a 35-D state tensor with mass at
    # index 13 and size at index 14 instead of 6/7 -- caller selects
    # which layout to use via the ``state_schema`` constructor argument.
    # See ``STATE_SCHEMAS`` below.
    #
    # KEPT as class constants for backward-compat with code that imported
    # ``PhysicsQAGenerator.MASS_IDX`` directly (some old callers do this
    # to write helpers that duplicate the schema). They are also assigned
    # at instance level in ``__init__`` so per-generator state-schema
    # selection actually takes effect on the answer methods (which all
    # reach through ``self.MASS_IDX`` / ``self.RADIUS_IDX``).
    POSITION_IDX = slice(0, 3)  # x, y, z
    VELOCITY_IDX = slice(3, 6)  # vx, vy, vz
    MASS_IDX = 6  # mass (legacy default)
    RADIUS_IDX = 7  # radius (legacy default)

    # Schema registry. Keep additions here in lockstep with
    # ``data_generation/clevrer/clevrer_to_training_data.py`` (CLEVRER
    # 35-D producer) and ``physics_former/training/constants.py``
    # (STATE_DIM=35 reference).
    #
    # CLEVRER 35-D layout, from
    # ``clevrer_benchmark/benchmark_20_objects_physics_llm.py``:
    #   [0:3]   position (x, y, z)
    #   [3:6]   velocity (vx, vy, vz)
    #   [6:10]  quaternion (qx, qy, qz, qw)
    #   [10:13] angular velocity
    #   [13]    mass
    #   [14]    size  (radius equivalent)
    #   [15:18] color RGB
    #   [18]    shape id
    #   [19]    is_static flag
    #   [20]    friction
    #   [21:35] additional material / shape one-hots
    #
    # Position and velocity slices are identical between schemas so
    # position-only and velocity-only questions work without picking a
    # schema -- it is only the mass / radius questions that produce
    # silently wrong answers when the schema is mis-selected.
    STATE_SCHEMAS = {
        'legacy': {
    ...
    def __init__(
        ...
        Args:
            ...
            state_schema: Which physical-feature layout the input state
                tensors use. ``'legacy'`` (default) matches the Phase 1-2
                synthetic physics dataset (mass=6, radius=7).
                ``'clevrer'`` matches the 35-D tensor produced by
                ``clevrer_benchmark.scene_converter.clevrer_scene_to_state_tensor``
                (mass=13, radius=14). Position and velocity indices are
                identical across schemas so position/velocity-only
                question types work either way; mass and radius
                questions silently produce wrong answers if the schema
                is mis-selected. New schemas can be added to
                ``STATE_SCHEMAS`` without touching answer methods --
                they all reach through ``self.MASS_IDX`` /
                ``self.RADIUS_IDX``.
        """
        if state_schema not in self.STATE_SCHEMAS:
            raise ValueError(
                f"Unknown state_schema={state_schema!r}; supported: "
                f"{list(self.STATE_SCHEMAS.keys())}"
            )
        schema = self.STATE_SCHEMAS[state_schema]
        # Override the class-level defaults so every answer method
        # using ``self.POSITION_IDX`` / ``self.MASS_IDX`` / etc. reads
        # the per-instance schema. Class-level constants stay at the
        # legacy values so direct imports keep working.
        self.state_schema = state_schema
        self.POSITION_IDX = schema['POSITION_IDX']
        self.VELOCITY_IDX = schema['VELOCITY_IDX']
        self.MASS_IDX = schema['MASS_IDX']
        self.RADIUS_IDX = schema['RADIUS_IDX']
